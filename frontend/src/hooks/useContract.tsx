...
export const useIERC721 = (address: string) => {
  const { library } = useActiveWeb3React();
  return useMemo(() => library && getIERC721Contract(address, library.getSigner()), [address, library]);
}

export const useWyvernStaticMarket = () => {
  const { library } = useActiveWeb3React();
  return useMemo(() => library && getWyvernStaticMarketContract(library.getSigner()), [library]);
}

export const useWyvernExchange = () => {
  const { library } = useActiveWeb3React();
  return useMemo(() => library && getWyvernExchangeContract(library.getSigner()), [library]);
}

export const useERC20Token = () => {
  const { library } = useActiveWeb3React();
  return useMemo(() => library && getERC20TokenContract(library.getSigner()), [library]);
}
